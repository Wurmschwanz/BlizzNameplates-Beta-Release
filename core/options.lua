...
function BNP:CreateOptions()
  ...
  local healthPercent = CreateCheck(frame, "Health %", -316, function()
    BNP_DB.healthPercent = this:GetChecked() and true or false
    if BNP.RefreshHealthPercent then BNP:RefreshHealthPercent() end
  end)
  frame.healthPercentCheck = healthPercent

  local targetFocus = CreateCheck(frame, "Target Glow", -346, function()
  ...
  glowColorLabel:SetPoint("TOPLEFT", frame, "TOPLEFT", 28, -378)
  ...
  frame.glowColorButtons = {}

  local i
  for i = 1, table.getn(colorKeys) do
    local data = colorKeys[i]
    ...
    btn:SetText(data[1])
    btn.BNPColorKey = data[2]

    btn:SetScript("OnClick", function()
      BNP_DB.targetGlowColor = this.BNPColorKey
      if BNP.RefreshTargetFocus then BNP:RefreshTargetFocus() end
    end)

    frame.glowColorButtons[i] = btn
  end

  local targetOnTop = CreateCheck(frame, "Target Plate on Top", -466, function()
  ...
  note:SetPoint("TOPLEFT", frame, "TOPLEFT", 28, -506)
  note:SetWidth(235)
...
function BNP:SyncOptions()
  ...
  frame.healthPercentCheck:SetChecked(self:IsHealthPercentEnabled())
