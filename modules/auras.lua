...
-- Aura frames must not inherit the projected Z-order of one particular mob.
-- Otherwise the aura row of a foreground plate can cover the complete
-- healthbar/name of another plate behind it. Keep every aura on one shared,
-- native WorldFrame background layer and only anchor it to its owner plate.
local AuraWorldLayer = CreateFrame("Frame", nil, WorldFrame)
AuraWorldLayer:SetAllPoints(WorldFrame)
AuraWorldLayer:SetFrameStrata("BACKGROUND")
AuraWorldLayer:SetFrameLevel(0)
AuraWorldLayer:Show()
...
local function GetAuraFrameLevel(plate)
  -- Auras belong visually to their own nameplate, but another mob's complete
  -- nameplate (especially its healthbar/name) must win when the two projected
  -- plates overlap. The old hard-coded levels 40/41 put every aura row above
  -- healthbars, Blizzard NPC dialogue and other low-level world UI children.
  -- Keep aura frames one level below BNP's healthbar instead.
  local healthLevel = 1
  if plate and plate.healthbar and plate.healthbar.GetFrameLevel then
    healthLevel = tonumber(plate.healthbar:GetFrameLevel()) or healthLevel
  end
  return math.max(0, healthLevel - 1)
end
...
local function InstallAuraHideGuard(plate)
  ...
  plate:SetScript("OnHide", function()
    ...
    HidePlateAuraFrames(this or plate)
  end)
...
local function CreateAuraContainer(plate)
  ...
  local container = CreateFrame("Frame", nil, AuraWorldLayer)
...
local function SyncAuraRemoval(unit, guid, now, snapshot)
  ...
  if unit == "target" then
    currentTargetGUID = GetTargetGUID()
    if UnitIsPlayer and UnitIsPlayer("target") then
    ...
    if plate:IsShown() and (hasPending or hasPendingAoE or doRemovalScan) then
      ...
      if guid and token then
        ...
        if doRemovalScan and not isCurrentTarget then
          local snapshot = nil
          if HAS_SHARED_AURAS or CacheHasTrackedAuras(BNP.guidAuras[guid]) then
            snapshot = CaptureLiveDebuffSnapshot(token, now)
          end

          SyncSharedAuras(token, guid, now, snapshot)
          ...
          SyncAuraRemoval(token, guid, now, snapshot)
        end
      end
    end
