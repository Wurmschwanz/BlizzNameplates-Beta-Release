...
local function FindSharedAuraOnUnit(unit, def, snapshot)
  if not unit or not def then return nil, nil, nil, nil, nil end
  ...
  for i = 1, maxIndex do
    ...
    if sharedMatches == true or (sharedMatches == nil and AuraMatches(def, auraName, texture)) then
      ...
      return auraSpellID, texture, duration, expires, tonumber(a2) or 0
    end
  end
...
local function SyncShadowVulnerabilityPresence(unit, guid, now, def, auraSpellID, texture, liveStacks)
  local cache = BNP.guidAuras[guid]
  ...
  if auraSpellID or texture then
    if not state then
      state = {
        present = true,
        lastSeen = now,
        expires = now + (def.duration or 10),
      }
      BNP.svStateByGUID[guid] = state
    else
      state.present = true
      state.lastSeen = now
      if not state.expires then state.expires = now + (def.duration or 10) end
    end
...
local function SyncSharedAuras(unit, guid, now, snapshot)
  ...
  for _, def in ipairs(AURA_DEFS) do
    if def.shared then
      local auraSpellID, texture, liveDuration, liveExpires, liveStacks = FindSharedAuraOnUnit(unit, def, snapshot)
      ...
      if def.key == "shadow_vulnerability" then
        SyncShadowVulnerabilityPresence(unit, guid, now, def, auraSpellID, texture, liveStacks)
      else
        local cache = BNP.guidAuras[guid]
...
local function RefreshSharedShadowVulnerability(guid)
  ...
  -- A proven SV application/refresh event always starts a fresh local 10s
  -- clock. Do not reuse UnitDebuff expiration data here: SuperWoW 1.12 does
  -- not reliably expose a changed expiration when an already-active aura is
  -- refreshed.
  aura.duration = def.duration or 10
  local state = BNP.svStateByGUID[guid]
  if not state then
  ...
  state.present = true
  state.expires = now + aura.duration
...
-- Shadow Vulnerability refresh: proven v1.0.5 behavior ----------------------
--
...
--
-- Keep that behavior here deliberately. It can over-estimate a refresh when
-- the talent fails to proc, but it avoids the broken mid-duration timer on
-- 1.12/SuperWoW clients that do not expose a distinct re-application event.
