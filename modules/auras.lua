...
local function ClearAuraStateByName(name)
  ...
  -- Death chat only gives us a unit name, not the exact GUID.
  -- If multiple living/cached units share that same name, clearing all of them
  -- would incorrectly remove valid debuffs from the survivors.
  local matchGUID = nil
  local matches = 0
  local guid, knownName

  for guid, knownName in pairs(BNP.guidNames or {}) do
    if knownName == name and BNP.guidAuras[guid] then
      matches = matches + 1
      matchGUID = guid

      if matches > 1 then
        -- Ambiguous same-name death: do nothing here.
        -- Exact GUID death cleanup will handle units we can identify safely.
        return
      end
    end
  end
    ...
    if type(pending) == "table" then
      if pending.judgementProbe then
        ...
        elseif now - pending.created > PENDING_TIMEOUT then
          ...
          -- A completed cast is NOT proof that a hostile aura landed.
          -- Dodge, miss, resist, immune and evade can all occur after CAST.
          -- If the exact aura never appears on this GUID, never create a timer.
          pendingForGUID[key] = nil
        end
      end
    end
    ...
    if type(aura) == "table" and aura.spellID and aura.expires then
      ...
      if age >= REMOVAL_GRACE then
        if present[aura.spellID] then
          -- Any positive sighting immediately resets transient-missing state.
          aura.missingScans = nil
        elseif exactRemoval then
          -- CC/roots/traps must disappear immediately when broken/dispelled.
          cache[key] = nil
        elseif playerRemoval then
          -- Normal player DoTs/curses use conservative removal: require
          -- several consecutive missing scans before treating the aura as gone.
