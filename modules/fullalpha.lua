...
local function DesiredAlpha(plate)
  ...
  if classicTargetAvailable then
    ...
    -- unnecessary. Target changes can update alpha in the event itself.
    if not classicTargetPlate then return 1 end

    local plateGUID = GetVerifiedPlateGUID(plate)
    if not plateGUID or plateGUID == targetGUID then return 1 end

    -- The exact ClassicAPI target frame is forced back to alpha 1 immediately
    -- after this value is applied. Therefore a stale valid GUID on the target
    -- cannot leave it dimmed, while same-name non-targets can still be faded.
...
local function ApplyTargetAlpha(plate)
  ...
  local targetGUID = GetTargetGUID()
  if targetGUID then
    local targetPlate, available = GetClassicAPITargetPlate(targetGUID)
    if available and targetPlate and targetPlate.GetAlpha and targetPlate.SetAlpha then
