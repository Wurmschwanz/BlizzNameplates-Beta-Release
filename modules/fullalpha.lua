...
local function KeepShaguGlowLayering(plate)
  if not plate then return end

  local anchor = plate.healthbar
  local visualParent = GetGlowVisualParent(plate)
  if not anchor or not visualParent then return end

  -- ShaguPlates creates its health StatusBar at FrameLevel 4 specifically
  -- "to keep above glow". Reproduce that relationship relative to BNP's
  -- visual wrapper, including when Target Plate on Top changes its level.
  if anchor.SetFrameLevel and visualParent.GetFrameLevel then
    local wanted = (visualParent:GetFrameLevel() or 0) + 4
    if anchor:GetFrameLevel() ~= wanted then
      anchor:SetFrameLevel(wanted)
    end
  end
end
...
local function EnsureTargetGlow(plate)
  ...
  local anchor = plate.healthbar or plate
  local visualParent = GetGlowVisualParent(plate)
  if not anchor or not visualParent then return nil end

  -- Direct port of ShaguPlates' target glow:
